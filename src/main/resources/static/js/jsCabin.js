...
function pintarRespuestaCabin(response){
    ...
    myTable+="<tr>";
        ...
        myTable+='<td><button class="botonsecundario" onclick="borrar(' + response[i].id + ')">Borrar Cabin!</button></td>';
        myTable+='<td><button class="botonsecundario" onclick="actualizar(' + response[i].id + ')">Actualizar Cabin!</button></td>';
        myTable+="</tr>";
    }
    myTable+="</table>";
...
function cargarDatosCabin(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.109.197:8080/api/Cabin/"+id,
        //url: "http://129.151.109.197:8080/api/Cabin/" + id,
        type: 'GET',
